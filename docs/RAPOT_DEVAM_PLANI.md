...
- **Son çalışma:** **P1-G1 tamamlandı ve 11 Eylül 20:56:32 UTC’de üretimde doğrulandı.** API, bot, middleware ve frontend `ccd61544ede603eb064d871a4ec797c5057307f4` imajlarında. Yerel/Linux 710/710 test uyarısız; exact 115/115 bağımlılık taramasında sıfır bilinen advisory. Üretimde 13/13 auth kontrolü, HTTPS/SSR, veri korunumu ve servis hazırlığı geçti. Ücretli yükseltme yapılmadı; yalnız ayrıca onaylanan üç eski sunucu yedeği, doğrulanmış kalıcı bilgisayar kopyaları korunarak kaldırıldı.
- **Uygulama durumu:** P0-1–P0-4, P1-1, P1-2, P1-4, P1-5, P1-6, P1-7, P1-G1, P2-1 ve P2-3 doğrulandı; P2-2 belge/karar kapsamı tamam. P1-G1 yerel Python 710/710 uyarısız; Linux CI aynı 710 vakayı ve frontend 95 test/lint/type/build/standalone ile container kabulünü geçti. Üretim kabulü yeni tam tarama, etkileşimli tarayıcı veya yük testi değildir. İç worktree ve 12 wrapper koruma kararı değişmedi.
...
- **Uzak doğrulama:** `ccd61544` için [CI 34641121926](https://github.com/Rapto0/Rapot/actions/runs/34641121926) ve [imaj yayını 34641767731](https://github.com/Rapto0/Rapot/actions/runs/34641767731) başarılı. Üretimde API health 47 ms/200; `/`, `/signals`, `/alarms`, `/chart` SSR 200. Beş servis healthy/restart0; SignalFeed hazır, Binance/BIST sağlayıcıları başlamış. Scheduler 47,298 saniyede unknown → running oldu. 1.733.137 sinyal, 6 tarama geçmişi, 26.393 AI kaydı ve diğer ana sayılar; PostgreSQL kimliği ve 135/135/28/383 sayıları korundu. DRY_RUN/false/false ve AI kapalı; dış Windows HTTPS API/bot kontrolleri ayrı ayrı 203 ms/200.
...
- **Aktif iş:** **Sıradaki ana iş P2-4 — eşzamanlı API yükünde realtime gecikmesi.** P1-G1 üretim kabulü kapandı; P2-4 uygulamasına eşzamanlı DB sorgu/notify yükünü azaltma adımıyla başlandı. Sağlık timeout ve WSS gecikme kök nedenini kanıtlama ölçüm çalışması hâlâ açık; dağıtım kabulü bu adımda kapatılmıyor. P3-1 COMBO sıfır-değer ve Pine EMA/ATR sınırları açık. P0-G1/P1-G1 kapalı olsa da Bandit 15 MEDIUM/1.534 LOW bulgusu ve genel güvenlik report-only politikası ayrı kalır. Borsa/alarm dış kabulü kullanıcı kararıyla erteli.
...
- **Açık işler:** P2-4 ve P3 işleri; kullanıcı isteğiyle ertelenmiş gerçek TradingView alarm teslimi/filtre ve testnet emir kabulü. P1-G1 uygulama dağıtımı tamam. Son belge commit/CI ve canonical Git blob hash’leri `/root/rapot-ops/20260911-p1g1/documentation-release-record.json` kaydında tutulur; belge yayını uygulama restart’ı gerektirmez. Kabul sonrası boş disk **724.676.608 B (~691 MiB)**; bu tek dağıtımın kapasite kapıları geçti, sonraki imaj/yedek için yeniden ölçüm gerekir. Ücretli yükseltme yapılmayacak; diğer yedek/imaj/release’ler otomatik silinmeyecek.
...
**Önerilen sıra:** Başlangıç sırası P0-1 → P0-2 → P0-3 → P0-4, ardından P1-1/P1-2/P1-3 idi. P1-G1 tamamlandı; güncel sıra **P2-4 → P3**; P1-3 dış kabulü kullanıcı yeniden seçene kadar ertelidir.
...
**P1-G1 tamamlandı: ccd61544 üretimde; yerel/Linux 710/710, 115/115 bağımlılık taramasında sıfır bilinen advisory ve üretim auth 13/13 geçti. Ücretli yükseltme yapılmadı; yalnız onaylanan üç eski sunucu kopyası, doğrulanmış kalıcı PC yedekleri korunarak kaldırıldı. Taze SQLite yedeği/bağımsız restore, PG katalog kontrolü ve veri/servis kabulü tamam. Kalan disk yaklaşık 691 MiB; sonraki dağıtım öncesi yeniden bütçe gerekir. Sıradaki ana iş P2-4; P3 ve dış alarm/testnet/emir kabulü açık. DRY_RUN/trading=false/live=false korunuyor.**
