...
- **Son çalışma:** **P1-4 tamamlandı ve 2026-09-10'da üretime alındı.** Uygulama kaynağı `fce5d011ddaf6444be30735dc2a26129ade6c62e`; AI–sinyal ilişkisi, sync/async tarama geçmişi ve sonuçların UI gösterimi düzeltildi. Beş servis healthy/restart=0; HTTPS ve veri korunumu doğrulandı.
- **Uygulama durumu:** P0 işleri, P1-1, **P1-2, P1-4** ve P1-7 doğrulandı. **471/471 Python**, **15/15 frontend**, lint/typecheck/build geçti. 68 yeni Python ve 3 frontend testi eklendi. Mevcut `datetime.utcnow()` kullanımı daha fazla gerçek persistence yolu sınandığı için bu koşumda 193 warning üretti; P2-3'te açık.
...
- **Sıradaki adım:** **P1-5 — süreçler arası realtime ve scanner eşdeğerliği.** Salt okunur ön inceleme tamamlandı; uygulama başlamadı. Başlangıç noktası API tarafında ortak SQLite'dan yeni kayıtları okuyan görev ve frontend reconnect/REST toparlanması. Kullanıcının 10 Eylül erteleme talimatıyla borsa/alarm dış kabulü beklenmeden diğer işlere devam ediliyor.
...
| P1-5 | Süreçler arası realtime ve scanner eşdeğerliği | Bekliyor — ön inceleme ve uygulama sırası kaydedildi | Orta/büyük | P0-1, P1-2 |
...
**2026-09-10 ön incelemesi — uygulama başlamadı:** API ve bot ayrı süreçlerde aynı SQLite'ı kullanıyor; callback kaydı bot sürecine taşınmıyor. Sinyal socket'i frontend'de ticker açılışına bağlı; reconnect REST sorgularını yenilemiyor. Store aynı ID'yi tekrar ekleyebiliyor, eski realtime satırı sonradan REST'ten gelen özel etiketi gölgeleyebiliyor. Kline/trade callback'leri bootstrap'te bağlı değil. Async hesaplayıcılar ortak olsa da özel etiket/AI/ikinci kaynak ve sync BIST 90 saniye tazelik kapısı eşdeğer değil.

**Somut başlangıç sırası:** Önce API yaşam döngüsüne bağımsız SQLite okuma görevi ekle: `id > cursor ORDER BY id LIMIT batch_size`; sorgu event loop dışında, yayın API loop'unda await ile. Cursor sadece başarıyla işlenen son ID'ye ilerlesin; DB/yayın hatasında aynı batch yeniden denenebilsin. Başlangıç MAX değeri socket kabulünden önce alınsın; frontend ilk bağlantı/reconnect'te REST'i yenilesin, ID ile birleştirsin ve güncel REST satırını öncelesin. Sinyal bağlantısı ticker'dan bağımsız çalışsın; yavaş istemci tüm yayını kilitlemesin. Ardından kline/trade kayıt/cleanup bağlantısı; async BIST batch işleme/tazelik ve ortak özel etiket/AI yordamı ele alınmalı. `notify=False` yalnız bildirimleri susturmalı; P1-4 kesin sinyal ID sözleşmesi korunmalı.

**Taşıma sınırları / testler:** Mevcut otomatik ID ve SQLite tek-yazıcı düzeninde commit sırası izlenebilir; bu varsayım başka DB'ye taşınmaz. Başlangıç MAX ile API kapalıyken kaçanlar REST görünümünden toparlanır; mevcut limitlerle tüm tarihsel olayların teslimi garanti edilmez. `id` taraması sonradan etiket/AI güncellemesini görmez; olay/reconnect invalidation ve periyodik REST yenilemesi korunmalı. Kalıcı güncelleme olayları gerekirse aynı transaction'da outbox ayrı karar olur. DB restore/silme ve ID tekrar kullanımı için açık resync politikası gerekir. Testler: bağımsız DB bağlantılarında commit/rollback, batch sınırı/ID boşluğu, yayın/DB hatası, restart/reconnect, geç etiket güncellemesi, yinelenen olay, yavaş istemci, kapanışta görev/soket sızıntısı ve aynı OHLCV ile sync/async özel sinyal eşdeğerliği. Ön inceleme dışında P1-5 değişikliği/testi yapılmadı.
...
**P0-1–P0-4, P1-1, P1-2, P1-4 ve P1-7 tamamlandı. P1-3 dış alarm/emir kabulü kullanıcı isteğiyle ertelendi. Üretim fce5d01; Python 471/471, frontend 15/15, Linux CI/imaj yayını ve sunucu kabulü geçti. DRY_RUN/false/false ve veriler korundu. Sıradaki kod işi P1-5: API SQLite sinyal görevi + frontend reconnect/REST toparlanması; ön inceleme kaydı hazır, uygulama başlamadı. P3-1 EMA/ATR sınırı açık.**
