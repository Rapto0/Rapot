...
- **Son çalışma:** **P1-G1 yerel kabul, commit/push, Linux CI ve imaj yayını tamamlandı: 710/710 test uyarısız; 115/115 exact paket taramasında sıfır bilinen advisory. Disk kapasitesine bağlı üretim kabulü açık. Kullanıcı ücretli yükseltmeyi reddetti; üç eski arşivin özel bilgisayar kopyaları doğrulandı, yalnız sunucu kopyalarını silme onayı bekleniyor.** Önceki P2-3 uygulama, Linux CI/imaj ve üretim kabulü tamamlandı. API, bot, middleware ve frontend hâlâ `bacbfab814bb659f093a33733934b56f2bf5b19a` kaynağının imajlarında; dağıtım 11 Eylül **19:14:51 UTC**'de doğrulandı. UTC düzeltmeleri ve altı atıl frontend dosyasının kaldırılması üretimde; tam kaynak Ruff ve güvenlik sonuç/hata ayrımı CI kapsamında yayımlandı. İç worktree’nin dosyaları, `f752ee2` HEAD’i/kaydı ve P2-2’deki **12 wrapper koruma kararı** korunuyor.
- **Uygulama durumu:** P0-1–P0-4, P1-1, P1-2, P1-4, P1-5, P1-6, P1-7, P2-1 ve P2-3 doğrulandı; P2-2 belge/karar kapsamı tamam. P2-3 yerel Python **642/642, uyarısız (65,85 saniye)**; 21 yeni UTC ve 33 CI politika testi dahil. Frontend **95/95**, lint, TypeScript, production build ve standalone HTTP/RSC/WS geçti; aynı kaynak için Linux CI de başarılı. Üretim kabulü HTTPS/SSR, veri korunumu ve servis hazırlığını kapsar; tam tarama veya yeni etkileşimli tarayıcı kabulü değildir.
...
- **Uzak doğrulama:** `bacbfab8` için [CI 34636602751](https://github.com/Rapto0/Rapot/actions/runs/34636602751) ve [imaj yayını 34637100266](https://github.com/Rapto0/Rapot/actions/runs/34637100266) başarılı. Üretimde HTTPS `/api/health` **36 ms/200**, `/`, `/signals`, `/alarms`, `/chart` SSR **200**; SignalFeed hazır, Binance bağlı, BIST sağlayıcısı çalışıyor. Scheduler **43,781 saniyede unknown → running** oldu; başlangıçta `data_available=false`, yeni tam tarama kabulü yapılmadı. Ana sayılar korundu; PostgreSQL kimliği/sayıları ve DRY_RUN/false/false değişmedi.
...
- **Aktif iş:** **P1-G1 — bağımlılık güvenlik düzeltmesi**; yerel ve Linux CI 115/115 paket taramasında sıfır bilinen bulgu. Yerel kabul ve CI/imaj tamam; üretim kapasitesi açık engel. Ayrıntılar [P1-G1 kaydında](#p1-g1--python-bağımlılık-güvenliği). P2-4’e henüz geçilmedi. P0-G1 standalone Flask debug düzeltmesi P2-3 ile yayımlandı; yerel/CI Bandit HIGH=0. Tarama altyapısının geçmesi diğer bulguları kapatmaz. P2-4 eşzamanlı ekran/heartbeat ve mum gecikmesi açık; 11 Eylül yayın öncesi 15 saniye health timeout → 45 ms/200 gözleminin nedeni kanıtlanmadı. P3-1 COMBO sıfır-değer ve Pine EMA/ATR sınırları kayıtlı. Borsa/alarm dış kabulü kullanıcı kararıyla erteli.
...
- **Açık işler:** P1-G1 disk kapasitesine bağlı üretim kabulü; P2-4 ve P3 işleri; kullanıcı isteğiyle ertelenmiş gerçek TradingView alarm teslimi/filtre ve testnet emir kabulü. P2-3 uygulama dağıtımı tamam. Son belge commit/CI, Git blob hash’leri ve aktarım kanıtı sürüm bazında `/root/rapot-ops/20260911-p23/documentation-release-record.json` içinde tutulur; uygulama imajını değiştirmez veya restart gerektirmez. P1-G1 kapasite ölçümünde boş disk **872.407.040 bayt**; sonraki imaj/yedek öncesi kapasite yeniden kontrol edilmeli. Ücretli yükseltme yapılmayacak. Eski imajlar/yedekler korundu, veri/rollback otomatik temizlenmeyecek. Borsa emri gönderilmedi.
...
| P2-4 | Eşzamanlı API yükünde realtime gecikmesi | Bekliyor — ilk üretim ve 11 Eylül health zaman aşımı kaydedildi | Orta | P1-5 |
...
| P1-G1 | Kilitli Python bağımlılığı advisory incelemesi | Devam ediyor — 710/710, sıfır dependency bulgusu, CI/imaj başarılı; üretim kapasitesi açık | Orta | P2-3 raporu; uygulama/test bağımlılığı ayrımı |
...
**Önerilen sıra:** Başlangıç sırası P0-1 → P0-2 → P0-3 → P0-4, ardından P1-1/P1-2/P1-3 idi. Güncel sıra **P1-G1 → P2-4 → P3**; P1-3 dış kabulü kullanıcı yeniden seçene kadar ertelidir.
...
**Durum — 11 Eylül 2026: Doğrulandı.** `bacbfab814bb659f093a33733934b56f2bf5b19a` commit/push, Linux CI/imaj ve **19:14:51 UTC üretim kabulü tamam**; API/bot/middleware/frontend aynı kaynak sürümünde. Güvenlik bulguları report-only kalır; P1-G1 kapanmadı. Son belge commit/CI ve aktarım kanıtı ayrı operator kaydında tutulur; uygulama imajını değiştirmez.
...
**Durum (11 Eylül 2026):** İncele → düzelt → yerel doğrula → belgeyi güncelle, commit/push ve Linux CI/imaj yayını tamamlandı. **Üretim hâlâ `bacbfab8`; yeni bağımlılıklar sunucuya kurulmadı.** Disk kapasitesi dağıtım için açık engel; kullanıcı ücretli yükseltmeyi reddetti. P2-4'e geçilmedi.
...
Üç arşiv farklı inode/nlink1; fiziksel ayrılmış alan toplamı **606.580.736 bayt (~578,48 MiB)**. 20:20:39 UTC güncel boş alan **868.999.168 bayt**; üç kopyanın kaldırılması öngörülen alan ihtiyacını karşılayabilir, gerçek boş alan yeniden ölçülmeden pull yapılmaz. **Silme onayı istendi, henüz verilmedi; sunucu kopyaları hâlâ korunuyor.** Güncel P2-3 SQL gzip yedeği, P1-6 r2/r3/r4 yedekleri ve tüm eski imaj/release'ler bu işlemin dışındadır. P1-6 üç yolu zaten aynı inode'a bağlı olduğundan bunlarda ek tekilleştirme alan kazandırmaz.

**Hazır operator adımları:** Yalnız üç sabit arşiv için ayrı kullanıcı onayı ve bu manifest hash'ini zorunlu kılan silme yardımcısı yerelde hazır; 12 sentetik kontrol geçti, Linux'ta gerçek silme çalıştırılmadı. Dağıtım yardımcısı yeni imaj ölçümünü hash ile bağlar: taze yedek/kaynak zaten diskteyken ilk pull öncesi **>1.186.175.248 B**, backend doğrulandıktan sonra **>553.661.268 B**, iki imaj tamamlanınca **>553.648.128 B** boş alan ister. 15 sentetik kontrol/Python3.10 AST/Ruff ve bağımsız inceleme geçti; kapasite kapıları gerçek Docker disk zirvesi kabulü değildir. Kaynak hazırlayıcıdaki eski P2-3 OPS kontrolü yükleme öncesi düzeltildi; arşiv SHA/Git ve gerçek iki arşiv farkı aynı 17 dosya, yedek/yerel doğrulama/hazırlama/dağıtım dosya sözleşmeleri uyumlu. Operator'lar yüklenmedi, taze üretim yedeği/pull/deploy başlatılmadı.

**Açık kabul:** Yeni katmanlara göre ücretsiz kapasite çözümü; taze/bağımsız doğrulanmış yedek, kontrollü backend geçişi ve HTTPS/auth/veri korunumu. Mevcut üretim DRY_RUN/trading=false/live=false kalır. Dış TradingView alarmı ve testnet/gerçek emir kabulü kullanıcı kararıyla erteli.

### P2-4 — Eşzamanlı API yükünde realtime gecikmesi

**11 Eylül yayın öncesi ek gözlem:** Mevcut üretimde salt okunur `/api/health` isteği 15 saniyede zaman aşımına uğradı; sonraki tekrar yaklaşık **45 ms / HTTP200** döndü. Bu iki sonuç aynı yük/önbellek koşulu benchmark’ı değildir; kök neden veya kalıcı düzelme çıkarılmıyor. P2-3 araç/UTC çalışması P2-4’ü kapatmaz; eşzamanlı istek ve event-loop ölçümü hâlâ gerekli.

**P2-1 sonrası ek gözlem:** db98915 frontend ile chart URL/SSR doğru ve header hydration temiz. İlk kısa üretim gözleminde BTCUSDT mumları henüz yüklenmemişti; aynı mum yolu Nginx'te 21:15:50 UTC 200/100.656 bayt olarak tamamlandı. Ayrı tekrar gözleminde 1.000 mum/Binance görüldü. İstek başlangıç zamanı/önbellek/yük eşit koşullarda ölçülmedi; gecikme miktarı veya kök neden bu gözlemden çıkarılmıyor. API/bot değiştirilmeden yapılan yayın bu performans işini kapatmaz.

**P1-6 sırasında sağlık yolu öne alındı — P2-4 tamamı açık:** İlk yayının `/api/health` kabulü20s timeout/499 oldu ve otomatik rollback çalıştı. Eski sürümde aynı HTTPS sağlık isteği5,561s; ayrı API container sürecinde mevcut `get_table_stats()`2,526s; doğrudan salt okunur SQLite signals COUNT0,053s ve altı LIMIT0 toplam yaklaşık0,0004s ölçüldü. Ölçümler aynı soğuk cache/yük koşulunda alınmadı;20s HTTP gecikmesini tek başına SQL sayımına bağlamaz. Sağlık artık sonuçları kullanılmayan COUNT'lar yerine tek session/row okumayan şema probe'u, hata aktarımı ve thread offload kullanıyor. Diğer async REST sorguları, eşzamanlı UI/WS yükü ve genel gecikme hedefi bu dar düzeltmeyle tamamlanmış sayılmıyor. Kaynak incelemesinde P1-6'nın periyodik BIST/Binance/SignalFeed dosyalarını değiştirmediği ve mevcut ağ/DB beklemelerinin async/thread üzerinden çalıştığı doğrulandı.

**P1-6 üretim gözlemi:** 2cf05a8 ve Nginx IPv4 düzeltmesi sonrası ek UI yükü olmadan health0,029–0,048s, kline/trade akışı geçti. Ekranlar açıkken sinyal heartbeat'i31,297s penceresinde görülmedi; sekmeler kapatılınca aynı kontrol30,250s heartbeat ile geçti. İlk sonuç saklandı. Nginx'in kanıtlanmış IPv6 bağlantı reddi düzeltildi; bu sonuç eşzamanlı DB/UI yükünün veya sinyal istemcisi kapanış gecikmesinin kök nedenini açıklamaz. Başlangıç welcome/kapsam/scheduler sürelerini ayrı ölçmek de bu işte ele alınmalı. P2-3’teki 36 ms sağlık ve 43,781 saniyelik scheduler hazırlığı farklı koşulda yeni tekil gözlemlerdir; bu performans işini kapatmaz.

**Kanıt / neden:** P1-5 sonrası ilk HTTPS/WSS kabulünde health/status 200 ve feed ready iken, aynı anda root→signals ekranı yüklenmesi sırasında üç WS handshake'i yaklaşık 7,5 saniyede zaman aşımına uğradı. Geçici tarayıcı kapatılınca aynı betik/URL/TLS ile üç kanal geçti; servis restart olmadı. Zaman aşımını üreten sorgu henüz ölçülmedi. İlk kaynak incelemesindeki `/health` COUNT çağrıları tarihsel bulgudur; `2cf05a8` bu yolu COUNT çalıştırmayan şema probe'una çevirdi. `api/main.py::get_signals` async route'undan senkron SQLAlchemy `.all()` ve `/stats` içinden senkron aggregate çağrıları devam ediyor. Header dört özel-tag, sinyaller sayfası iki liste sorgusu açabiliyor. Market indices/overview sağlayıcıları zaten `asyncio.to_thread` kullanıyor; bunları kanıtsız biçimde doğrudan event-loop bloklayıcısı sayma. P1-6'da kaydedilen scheduler başlangıcı ile yerel kapsam sorgusu ölçümü farklı koşullardadır; welcome, kapsam kontrolü ve scheduler hazırlığı VPS üzerinde ayrı ölçülmeden başlangıç gecikmesi SQL/index sorununa bağlanmamalı.
...
**P1-G1 yerel kabul, ccd61544 commit/push, Linux CI/container ve imaj yayını tamam: 710/710 uyarısız, 115/115 exact bağımlılık taramasında sıfır bilinen advisory. P2-3/P0-G1 üretimde bacbfab8 olarak korunuyor. Kullanıcı ücretli yükseltmeyi reddetti; üç eski arşiv özel, kalıcı bilgisayar klasörüne kopyalandı ve bütünüyle doğrulandı. Sunucu kopyalarını silmek için ayrıca somut onay gerekli; silme yapılmadı. Sıradaki adım kapasite çözümü, taze bağımsız doğrulanmış yedek ve kontrollü üretim kabulü. P2-4/P3 ve P1-3 dış alarm/testnet/emir kabulü açık; DRY_RUN/false/false korunuyor. P1-G1 üretim kabulü bitmeden sonraki ana işe geçilmedi.**
