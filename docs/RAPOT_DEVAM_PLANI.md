...
- **Son çalışma:** **P1-G1 yerel düzeltme ve kabulü tamamlandı: 710/710 test uyarısız; 115/115 exact paket taramasında sıfır bilinen advisory. CI/imaj ve kapasite çözümüne bağlı üretim kabulü açık.** Önceki P2-3 uygulama, Linux CI/imaj ve üretim kabulü tamamlandı. API, bot, middleware ve frontend artık `bacbfab814bb659f093a33733934b56f2bf5b19a` kaynağının imajlarında; dağıtım 11 Eylül **19:14:51 UTC**'de doğrulandı. UTC düzeltmeleri ve altı atıl frontend dosyasının kaldırılması üretimde; tam kaynak Ruff ve güvenlik sonuç/hata ayrımı CI kapsamında yayımlandı. İç worktree’nin dosyaları, `f752ee2` HEAD’i/kaydı ve P2-2’deki **12 wrapper koruma kararı** korunuyor.
...
- **Açık işler:** P1-G1 CI/imaj ve disk kapasitesine bağlı üretim kabulü; P2-4 ve P3 işleri; kullanıcı isteğiyle ertelenmiş gerçek TradingView alarm teslimi/filtre ve testnet emir kabulü. P2-3 uygulama dağıtımı tamam. Son belge commit/CI, Git blob hash’leri ve aktarım kanıtı sürüm bazında `/root/rapot-ops/20260911-p23/documentation-release-record.json` içinde tutulur; uygulama imajını değiştirmez veya restart gerektirmez. Geçiş sonrası boş disk **884.736.000 bayt**; sonraki imaj/yedek öncesi kapasite yeniden kontrol edilmeli. Eski imajlar/yedekler korundu, veri/rollback otomatik temizlenmeyecek. Borsa emri gönderilmedi.
...
| P1-G1 | Kilitli Python bağımlılığı advisory incelemesi | Devam ediyor — 710/710 ve sıfır dependency bulgusu; CI/imaj/üretim kapasitesi açık | Orta | P2-3 raporu; uygulama/test bağımlılığı ayrımı |
...
**Durum (11 Eylül 2026):** İncele → düzelt → yerel doğrula → belgeyi güncelle tamamlandı. CI/imaj yayını bekliyor. **Üretim hâlâ `bacbfab8`; yeni bağımlılıklar sunucuya kurulmadı.** Disk kapasitesi dağıtım için açık engel; P2-4'e geçilmedi.
...
**Açık kabul:** Exact SHA Linux CI/container ve imaj yayını; yeni katmanlara göre kapasite çözümü; taze/bağımsız doğrulanmış yedek, kontrollü backend geçişi ve HTTPS/auth/veri korunumu. Mevcut üretim DRY_RUN/trading=false/live=false kalır. Dış TradingView alarmı ve testnet/gerçek emir kabulü kullanıcı kararıyla erteli.
...
**P1-G1 yerel kabulü tamam: 710/710 uyarısız, 115/115 exact bağımlılık taramasında sıfır bilinen advisory; .venv aynı lock'a eşit, Ruff218/218 temiz. P2-3/P0-G1 üretimde bacbfab8 olarak korunuyor. Sıradaki adım exact SHA CI/container ve imaj yayını, sonra yeni katman boyutlarına göre disk kapasitesi çözümü, taze doğrulanmış yedek ve kontrollü üretim kabulü. Yaklaşık 832MiB boş alanda yeni pull güvenli kabul edilmedi; eski imaj/yedek/release silinmedi. Ücretli kapasite değişimi yapılmadı. P2-4/P3 ve P1-3 dış alarm/testnet/emir kabulü açık; DRY_RUN/false/false korunuyor. P1-G1 üretim kabulü bitmeden sonraki ana işe geçilmedi.**
