...
- **Sıradaki iş:** **P1-G1 — 24 paket/203 advisory kaydının kullanım ve erişilebilirlik incelemesi**, P2-4’ten önce ele alınacak. P0-G1 standalone Flask debug düzeltmesi P2-3 ile yayımlandı; yerel/CI Bandit HIGH=0. Tarama altyapısının geçmesi diğer bulguları kapatmaz. P2-4 eşzamanlı ekran/heartbeat ve mum gecikmesi açık; 11 Eylül yayın öncesi 15 saniye health timeout → 45 ms/200 gözleminin nedeni kanıtlanmadı. P3-1 COMBO sıfır-değer ve Pine EMA/ATR sınırları kayıtlı. Borsa/alarm dış kabulü kullanıcı kararıyla erteli.
...
- **Açık işler:** P1-G1 güvenlik bulgusu incelemesi; P2-4 ve P3 işleri; kullanıcı isteğiyle ertelenmiş gerçek TradingView alarm teslimi/filtre ve testnet emir kabulü. P2-3 uygulama dağıtımı tamam. Son belge commit/CI, Git blob hash’leri ve aktarım kanıtı sürüm bazında `/root/rapot-ops/20260911-p23/documentation-release-record.json` içinde tutulur; uygulama imajını değiştirmez veya restart gerektirmez. Geçiş sonrası boş disk **884.736.000 bayt**; sonraki imaj/yedek öncesi kapasite yeniden kontrol edilmeli. Eski imajlar/yedekler korundu, veri/rollback otomatik temizlenmeyecek. Borsa emri gönderilmedi.
...
| P1-G1 | Kilitli Python bağımlılığı advisory incelemesi | Sıradaki — P2-4’ten önce; 24 paket/203 kaydın erişilebilirliği ve düzeltme planı ayrılacak | Orta | P2-3 raporu; uygulama/test bağımlılığı ayrımı |
...
**P2-3 ve P0-G1 tamamlandı: bacbfab8 tam kaynak CI/imaj kabulüyle 11 Eylül 19:14:51 UTC’de API/bot/middleware/frontend’e yayımlandı. Yerel Python 642/642 uyarısız, frontend 95/95 ve build/standalone geçti; üretim HTTPS/SSR, veri korunumu ve servis hazırlığı doğrulandı. Yeni SQL gzip bağımsız SQLite restore ile doğrulandı; PG dump yalnız katalog kontrolünden geçti. Eski yedek/imajlar, iç worktree ve 12 wrapper korunuyor; DRY_RUN/false/false, emir yok. Sıradaki P1-G1: 24 paket/203 advisory kaydının kullanım/erişilebilirlik incelemesi. P2-4/P3 ve P1-3 dış alarm/testnet/emir kabulü açık; scheduler running gözlemi yeni tam tarama kabulü değildir. Son belge commit/CI, Git blob hash’leri ve aktarım kanıtı sürüm bazında `/root/rapot-ops/20260911-p23/documentation-release-record.json` içinde tutulur; uygulama imajını değiştirmez veya restart gerektirmez.**
