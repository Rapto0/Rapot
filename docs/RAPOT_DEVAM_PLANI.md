...
- **Son çalışma:** **P2-4 `279aa9f` ile üretimde doğrulandı.** Onaylanan dört eski günlük kaldırılarak yaklaşık 288 MiB açıldı; diğer sekiz günlük ve özel bilgisayar arşivi korundu. İlk sürümdeki etiket sorgusu zaman aşımı, kısmi indeks ve tam sayım düzeltmesiyle giderildi. Python 735 geçti / 1 opt-in test atlandı; ayrı büyük veri ölçümü, CI/imaj yayını ve üretimde 24 REST / 3 WSS kontrolü geçti. Beş servis sağlıklı, restart sayıları sıfır; auth 13/13, ana kayıt sayıları ve PostgreSQL korundu. Küçük üretim örneklemi SLA veya gerçek emir kabulü değildir.
...
- **Sıradaki iş:** **P3-1: COMBO hesaplayıcısında geçerli sıfır ile eksik değeri ayırmak.** İlk dosya `frontend/src/lib/indicators.ts`, ardından yerel alarm regresyonu ve sabit OHLCV örnekleriyle motor farklarının ölçümü. P3-1 kod değişikliği başlamadı. Güncel uygulama `279aa9fea99b520e661b43f104a2bf4791893ac3`; üretim kanıtı `/root/rapot-ops/20260913-p24-index/deployment.json`. Son belge commit/CI ve Git blob aktarımı aynı dizindeki `documentation-release-record.json` ile takip edilir; belge yayını servis restart'ı gerektirmez.
...
- **Açık işler:** P3-1/P3-2 ve kullanıcı isteğiyle ertelenmiş gerçek TradingView alarm/filtre ve testnet emir kabulü. Ücretli yükseltme veya diğer yedek, imaj, release ve günlükler için toplu silme yapılmaz. İkinci P2-4 yayını kabulünde boş disk **580.993.024 B (yaklaşık 554 MiB)**; büyüyen DB/günlükler nedeniyle her yeni yayın öncesi yedek, katman ve çalışma payı yeniden ölçülmelidir.
...
**P2-4 `279aa9f` ile üretimde doğrulandı: 735 Python testi, ayrı büyük veri ölçümü, CI/imaj, sağlık/auth ve 24 REST / 3 WSS kabulü geçti. Onaylı dört eski günlük kaldırıldı; kalan sekiz günlük, özel arşiv ve geri dönüş kaynakları korundu. DRY_RUN/trading=false/live=false sürüyor. Sıradaki P3-1 işi `frontend/src/lib/indicators.ts` içindeki COMBO sıfır/eksik değer ayrımıdır; kod değişikliği başlamadı. Ücretli yükseltme yapılmayacak. Gerçek TradingView alarm/testnet/emir kabulü kullanıcı kararıyla erteli.**
