...
- **Son çalışma:** **P3-1 ilk adımı yerelde doğrulandı: frontend COMBO sıfır/eksik değer ve sonlu puanlama düzeltmesi.** Altı yeni gerçek hesaplayıcı/alarm testi eski kodda beş hata gösterdi; düzeltmeyle frontend **105/105**, lint, type-check, build ve standalone HTTP/WS kabulü geçti. CI/imaj yayını ve frontend üretim geçişi henüz tamamlanmadı. Mevcut üretim `279aa9f` ve P2-4 kabulü korunuyor; tüm P3-1 veya motor eşdeğerliği tamamlandı sayılmaz.
...
- **Aktif iş:** **P3-1 ilk frontend düzeltmesini CI ve kapasite kontrolünden sonra yayımlamak.** Sonraki kod işi sabit OHLCV örnekleriyle Python/TypeScript/Pine fark ölçümü ve Pine EMA/ATR sınırlarıdır. Mevcut backend ve frontend kaynakları `279aa9fea99b520e661b43f104a2bf4791893ac3`; son kabul `/root/rapot-ops/20260913-p24-index/deployment.json`. Önceki belge commit'i `eabad2d`, CI 34746193999 ve 11 dosyanın canonical aktarımı aynı dizindeki `documentation-release-record.json` içinde doğrulandı. Yeni frontend-only geçişte bileşen SHA'ları ayrı kaydedilecek.
...
| P3-1 | Backtest ve strateji eşdeğerliği | COMBO sıfır/eksik değer ilk adımı yerelde 105 testle doğrulandı; CI/yayın, motor/Pine ve backtest işleri açık | Büyük | Emir/veri doğruluğu, P1-3, P1-4 |
...
**P3-1 ilk COMBO düzeltmesi yerelde tamam: 105 test, lint/type/build/standalone geçti; CI ve frontend-only yayın açık. Üretim hâlâ P2-4 `279aa9f`, DRY_RUN/trading=false/live=false. Sonraki kod adımı motor fark ölçümü ve Pine EMA/ATR sınırları; tüm P3 tamamlanmış değil. Ücretli yükseltme yapılmayacak, diğer yedek/imaj/günlükler korunacak. Gerçek TradingView alarm/testnet/emir kabulü kullanıcı kararıyla erteli.**
