...
Tablodaki yollar `frontend/src/` altındadır. Silinmiş bileşenlerin tarihsel
adları bağlantı yerine kod olarak gösterilir; güncel karşılıkların bağlantıları
çalışan kaynaklara gider.
...
| [components/ui/connection-status.tsx](../frontend/src/components/ui/connection-status.tsx) | Kullanılmayan `Icon` değişkeninin kaldırılması | Main bu değişkeni ve kullanılmayan ikon/import yapılarını zaten kaldırmış durumda. |
...
| [lib/hooks/use-chart-data.ts](../frontend/src/lib/hooks/use-chart-data.ts) | Rastgele demo mum üretimini helper/lazy state ve requestAnimationFrame ile düzenleme | Bu helper gerçek sağlayıcı verisi değildir. Repo içindeki kaynak/test aramasında tüketici bulunmadı; aktif grafik `fetchCandles` kullanır. Demo lifecycle değişikliği seçilmedi. |
