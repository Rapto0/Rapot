...
Geçici SQLite/sentetik veriler kullanıldı; gerçek DB, PostgreSQL sunucusu veya borsaya bağlantı yoktu. Tekrarlı ana şema eklemesi, eski kayıtların bilinmeyen alanlarını koruma, eksik/eski revision reddi, otomatik stamp engeli ve seçili `0004–0006` dönüşümleri kapsanır. O P2-2 koşumunda `datetime.utcnow()` kullanımından **10 deprecation warning** alındı. Sonraki P2-3 değişikliği `infrastructure.time.utc_now_naive` ile UTC-naive sözleşmesini ve eski kayıt/API biçimlerini koruyarak bu kullanımları değiştirdi; şema veya veri dönüşümü eklenmedi. Güncel sonuç devam planındadır. Tam PostgreSQL zinciri için [CI](../.github/workflows/ci.yml) kanıtı ayrıca gereklidir.
