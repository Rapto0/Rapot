...
Güncel iş sırası, doğrulama ve yayın kayıtları [Rapot Devam Planı](../docs/RAPOT_DEVAM_PLANI.md)
içindedir. P2-1 frontend kabulü tamamlandı; COMBO sıfır-değer/Pine hesaplama sınırları
P3-1'de, eşzamanlı ekran yükü ve API gecikmesi P2-4'te ayrıca takip edilir.
...
[`next.config.ts`](next.config.ts) içindeki `API_PROXY_TARGET` ve
...
`npm test`, [package.json](package.json) içindeki Node test dosyalarını çalıştırır.
